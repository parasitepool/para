use super::*;

#[derive(Debug, Clone, PartialEq, Default)]
pub struct ClientReconnect {
    pub hostname: Option<String>,
    pub port: Option<u16>,
    pub wait_time: Option<u32>,
}

impl Serialize for ClientReconnect {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        if self.hostname.is_none() && self.port.is_none() && self.wait_time.is_none() {
            let seq = serializer.serialize_seq(Some(0))?;
            seq.end()
        } else {
            let mut seq = serializer.serialize_seq(Some(3))?;
            seq.serialize_element(self.hostname.as_deref().unwrap_or(""))?;
            seq.serialize_element(&self.port.unwrap_or(0))?;
            seq.serialize_element(&self.wait_time.unwrap_or(0))?;
            seq.end()
        }
    }
}

impl<'de> Deserialize<'de> for ClientReconnect {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let values: Vec<Value> = Deserialize::deserialize(deserializer)?;

        let hostname = values
            .first()
            .and_then(Value::as_str)
            .filter(|s| !s.is_empty())
            .map(String::from);

        let port = values
            .get(1)
            .and_then(Value::as_u64)
            .map(|p| p as u16)
            .filter(|&p| p != 0);

        let wait_time = values
            .get(2)
            .and_then(Value::as_u64)
            .map(|w| w as u32)
            .filter(|&w| w != 0);

        Ok(ClientReconnect {
            hostname,
            port,
            wait_time,
        })
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn default_serializes_to_empty_array() {
        let v = serde_json::to_value(ClientReconnect::default()).unwrap();
        assert_eq!(v, serde_json::json!([]));
    }

    #[test]
    fn roundtrip() {
        #[track_caller]
        fn case(reconnect: ClientReconnect, json: &str) {
            let serialized = serde_json::to_string(&reconnect).unwrap();
            assert_eq!(
                serde_json::from_str::<Value>(&serialized).unwrap(),
                serde_json::from_str::<Value>(json).unwrap(),
            );

            let back: ClientReconnect = serde_json::from_str(&serialized).unwrap();
            assert_eq!(back, reconnect);
        }

        case(ClientReconnect::default(), "[]");

        case(
            ClientReconnect {
                hostname: Some("foo".into()),
                port: Some(3333),
                wait_time: Some(5),
            },
            r#"["foo", 3333, 5]"#,
        );

        case(
            ClientReconnect {
                hostname: Some("bar".into()),
                port: None,
                wait_time: None,
            },
            r#"["bar", 0, 0]"#,
        );
    }

    #[test]
    fn deserialize_partial() {
        #[track_caller]
        fn case(json: &str, expected: ClientReconnect) {
            let parsed: ClientReconnect = serde_json::from_str(json).unwrap();
            assert_eq!(parsed, expected);
        }

        case(r#"["foo"]"#, ClientReconnect {
            hostname: Some("foo".into()),
            port: None,
            wait_time: None,
        });

        case(r#"["foo", 3333]"#, ClientReconnect {
            hostname: Some("foo".into()),
            port: Some(3333),
            wait_time: None,
        });
    }

    #[test]
    fn deserialize_zeroes_as_none() {
        let reconnect: ClientReconnect = serde_json::from_str(r#"["", 0, 0]"#).unwrap();
        assert_eq!(reconnect, ClientReconnect::default());
    }
}
