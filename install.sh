...
help() {
  cat <<'EOF'
Install a binary release of para hosted on GitHub
...
if [ -z "${tag-}" ]; then
  tag=$(
    download https://api.github.com/repos/parasitepool/para/releases/latest - |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
if [ "$extension" = "zip" ]; then
  download "$archive" "$td/para.zip"
  unzip -jd "$td" "$td/para.zip"
else
  download "$archive" - | tar --directory "$td" --strip-components 1 -xz
fi

if [ -e "$dest/para" ] && [ "$force" = false ]; then
  err "\`$dest/para\` already exists"
else
  mkdir -p "$dest"
  cp "$td/para" "$dest/para"
  chmod 755 "$dest/para"
fi
