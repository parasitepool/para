...
void mkstamp(char* stamp, size_t siz) {
    ...
    hroff = minoff / 60;
    if (minoff % 60) {
        snprintf(tzinfo, sizeof(tzinfo), "%c%02ld:%02ld", tzch, hroff, minoff % 60);
    } else {
        snprintf(tzinfo, sizeof(tzinfo), "%c%02ld", tzch, hroff);
    }
    snprintf(
        stamp, siz, "[%d-%02d-%02d %02d:%02d:%02d%s]", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
        tm.tm_min, tm.tm_sec, tzinfo);
}

static struct option long_options[] = {{"counter", no_argument, 0, 'c'},        {"help", no_argument, 0, 'h'},
                                       {"loglevel", required_argument, 0, 'l'}, {"name", required_argument, 0, 'n'},
                                       {"sockname", required_argument, 0, 'N'}, {"proxy", no_argument, 0, 'p'},
                                       {"sockdir", required_argument, 0, 's'},  {"timeout1", required_argument, 0, 't'},
                                       {"timeout2", required_argument, 0, 'T'}, {0, 0, 0, 0}};
    ...
    while ((c = getopt_long(argc, argv, "chl:N:n:ps:t:T:", long_options, &i)) != -1) {
        switch (c) {
            ...
            case 'l':
                ...
                if (msg_loglevel < LOG_EMERG || msg_loglevel > LOG_DEBUG) {
                    quit(
                        1,
                        "Invalid loglevel: %d (range %d"
                        " - %d)",
                        msg_loglevel, LOG_EMERG, LOG_DEBUG);
                }
                break;
