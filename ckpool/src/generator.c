...
retry:
    ...
    if (!proxi->no_params) {
        JSON_CPACK(req, "{s:i,s:s,s:[s]}", "id", 0, "method", "mining.subscribe", "params", PACKAGE "/" VERSION);
        /* Then try without any parameters */
    } else {
        JSON_CPACK(req, "{s:i,s:s,s:[]}", "id", 0, "method", "mining.subscribe", "params");
    }
...
static bool passthrough_stratum(connsock_t* cs, proxy_instance_t* proxi) {
    ...
    JSON_CPACK(req, "{ss,s[s]}", "method", "mining.passthrough", "params", PACKAGE "/" VERSION);
...
static bool node_stratum(connsock_t* cs, proxy_instance_t* proxi) {
    ...
    JSON_CPACK(req, "{ss,s[s]}", "method", "mining.node", "params", PACKAGE "/" VERSION);
...
static bool send_version(proxy_instance_t* proxi, json_t* val) {
    ...
    JSON_CPACK(json_msg, "{sossso}", "id", id_val, "result", PACKAGE "/" VERSION, "error", json_null());
...
static bool send_pong(proxy_instance_t* proxi, json_t* val) {
    ...
    JSON_CPACK(json_msg, "{sossso}", "id", id_val, "result", "pong", "error", json_null());
    ret = send_json_msg(&proxi->cs, json_msg);
    json_decref(json_msg);
    return ret;
}
...
create_subproxy(ckpool_t* ckp, gdata_t* gdata, proxy_instance_t* proxi, const char* url, const char* baseurl) {
...
static bool parse_reconnect(proxy_instance_t* proxy, json_t* val) {
    ...
    if (new_url && strlen(new_url) && new_port) {
        ...
        if (!dot_pool) {
            LOGWARNING("Denied stratum reconnect request from server without domain %s", proxy->url);
            goto out;
        }
        dot_reconnect = strchr(new_url, '.');
        if (!dot_reconnect) {
            LOGWARNING("Denied stratum reconnect request to url without domain %s", new_url);
            goto out;
        }
        len = strlen(dot_reconnect);
        if (strncmp(dot_pool, dot_reconnect, len)) {
            LOGWARNING("Denied stratum reconnect request from %s to non-matching domain %s", proxy->url, new_url);
            goto out;
        }
...
static void send_diff(ckpool_t* ckp, proxy_instance_t* proxi) {
    ...
    JSON_CPACK(json_msg, "{sIsisf}", "proxy", proxy->id, "subproxy", proxi->subid, "diff", proxi->diff);
...
static void send_notify(ckpool_t* ckp, proxy_instance_t* proxi, notify_instance_t* ni) {
    ...
    JSON_CPACK(
        json_msg, "{sIsisisssisssssosssssssb}", "proxy", proxy->id, "subproxy", proxi->subid, "jobid", ni->id64,
        "prevhash", ni->prevhash, "coinb1len", ni->coinb1len, "coinbase1", ni->coinbase1, "coinbase2", ni->coinbase2,
        "merklehash", merkle_arr, "bbversion", ni->bbversion, "nbit", ni->nbit, "ntime", ni->ntime, "clean", ni->clean);
...
static bool parse_method(ckpool_t* ckp, proxy_instance_t* proxi, const char* msg) {
    ...
    if (!val) {
        if (proxi->global) {
            LOGWARNING(
                "JSON decode of proxy %d:%s msg %s failed(%d): %s", proxi->id, proxi->url, msg, err.line, err.text);
        } else {
            LOGNOTICE(
                "JSON decode of proxy %d:%s msg %s failed(%d): %s", proxi->id, proxi->url, msg, err.line, err.text);
        }
...
static bool auth_stratum(ckpool_t* ckp, connsock_t* cs, proxy_instance_t* proxi) {
    ...
    JSON_CPACK(req, "{s:i,s:s,s:[s,s]}", "id", 42, "method", "mining.authorize", "params", proxi->auth, proxi->pass);
    ret = send_json_msg(cs, req);
    json_decref(req);
    if (!ret) {
...
static void send_subscribe(ckpool_t* ckp, proxy_instance_t* proxi) {
    ...
    JSON_CPACK(
        json_msg, "{ss,ss,ss,ss,sI,si,ss,si,sb,si}", "baseurl", proxi->baseurl, "url", proxi->url, "auth", proxi->auth,
        "pass", proxi->pass, "proxy", proxi->id, "subproxy", proxi->subid, "enonce1", proxi->enonce1, "nonce2len",
...
static void* proxy_send(void* arg) {
    ...
    while (42) {
        ...
        JSON_CPACK(
            val, "{s[soooo]soss}", "params", subproxy->auth, jobid, json_object_dup(msg->json_msg, "nonce2"),
            json_object_dup(msg->json_msg, "ntime"), json_object_dup(msg->json_msg, "nonce"), "id",
            json_object_dup(msg->json_msg, "id"), "method", "mining.submit");
...
static void suggest_diff(ckpool_t* ckp, connsock_t* cs, proxy_instance_t* proxy) {
    ...
    JSON_CPACK(req, "{s:i,s:s, s:[I]}", "id", 41, "method", "mining.suggest", "params", ckp->mindiff);
    ret = send_json_msg(cs, req);
    json_decref(req);
    if (!ret) {
...
static bool proxy_alive(ckpool_t* ckp, proxy_instance_t* proxi, connsock_t* cs, bool pinging) {
...
static void* proxy_recv(void* arg) {
    ...
    while (42) {
        ...
        if (likely(ret > 0)) {
            ...
            LOGNOTICE("Proxy %d:%d %s failed to epoll in proxy_recv", proxi->id, subproxy->subid, subproxy->url);
            hup = true;
        }
...
static void send_list(gdata_t* gdata, const int sockd) {
    ...
    HASH_ITER(hh, gdata->proxies, proxy, tmp) {
        JSON_CPACK(
            val, "{si,sb,si,ss,ss,sf,sb,sb,si}", "id", proxy->id, "global", proxy->global, "userid", proxy->userid,
...
static void send_sublist(gdata_t* gdata, const int sockd, const char* buf) {
    ...
    HASH_ITER(sh, proxy->subproxies, subproxy, tmp) {
        JSON_CPACK(
            res, "{si,ss,ss,sf,sb,sb}", "subid", subproxy->id, "auth", subproxy->auth, "pass", subproxy->pass, "diff",
...
static proxy_instance_t*
__add_userproxy(ckpool_t* ckp, gdata_t* gdata, const int id, const int userid, char* url, char* auth, char* pass) {
    proxy_instance_t* proxy;
...
static void
add_userproxy(ckpool_t* ckp, gdata_t* gdata, const int userid, const char* url, const char* auth, const char* pass) {
    proxy_instance_t* proxy;
...
static void parse_addproxy(ckpool_t* ckp, gdata_t* gdata, const int sockd, const char* buf) {
    ...
    prepare_proxy(proxy);
    if (global) {
        JSON_CPACK(res, "{si,ss,ss,ss}", "id", proxy->id, "url", url, "auth", auth, "pass", pass);
    } else {
        JSON_CPACK(
            res, "{si,ss,ss,ss,si}", "id", proxy->id, "url", url, "auth", auth, "pass", pass, "userid", proxy->userid);
    }
out:
    if (val)
        json_decref(val);
...
static void parse_delproxy(ckpool_t* ckp, gdata_t* gdata, const int sockd, const char* buf) {
    ...
    JSON_CPACK(
        res, "{si,ss,ss,ss,ss}", "id", proxy->id, "url", proxy->url, "baseurl", proxy->baseurl, "auth", proxy->auth,
        "pass", proxy->pass);
...
static void send_subproxystats(gdata_t* gdata, const int sockd) {
    ...
    HASH_ITER(hh, gdata->proxies, parent, tmp) {
        ...
        JSON_CPACK(val, "{si,so}", "id", parent->id, "subproxy", subarr_val);
