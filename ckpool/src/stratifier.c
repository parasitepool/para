...
static void generate_coinbase(ckpool_t* ckp, workbase_t* wb) {
    ...
    *u64 = htole64(g64);
    ...
    if (!ckp->btcsolo) {
        ...
        coinbase_len =
            wb->coinb1len + ckp->nonce1length + ckp->nonce2length + wb->coinb2len + sdata->txnlen + wb->coinb3len + 1;
        coinbase = ckzalloc(coinbase_len);
        memcpy(coinbase, wb->coinb1bin, wb->coinb1len);
        offset += wb->coinb1len;
        offset += ckp->nonce1length + ckp->nonce2length;
...
static json_t* generate_workinfo(ckpool_t* ckp, const workbase_t* wb, const char* func) {
    ...
    JSON_CPACK(
        val, "{sI,ss,ss,ss,ss,ss,ss,ss,ss,sI,so,ss,ss,ss,ss}", "workinfoid", wb->id, "poolinstance", ckp->name,
        "transactiontree", wb->txn_hashes, "prevhash", wb->prevhash, "coinbase1", wb->coinb1, "coinbase2", wb->coinb2,
        "version", wb->bbversion, "ntime", wb->ntime, "bits", wb->nbit, "reward", wb->coinbasevalue, "merklehash",
        json_deep_copy(wb->merkle_array), "createdate", cdfield, "createby", "code", "createcode", func, "createinet",
        ckp->serverurl[0]);
    return val;
}
...
static bool add_txn(ckpool_t* ckp, sdata_t* sdata, txntable_t** txns, const char* hash, const char* data, bool local) {
    bool        found = false;
    txntable_t* txn;
...
static txntable_t* wb_merkle_bin_txns(ckpool_t* ckp, sdata_t* sdata, workbase_t* wb, json_t* txn_array, bool local) {
 ...
 * client_id */
static void downstream_json(sdata_t* sdata, const json_t* val, const int64_t client_id, const int prio) {
    stratum_instance_t* client;
    ckmsg_t*            bulk_send = NULL;
    int                 messages = 0;
...
static void add_remote_blockdata(ckpool_t* ckp, json_t* val, const int cblen, const char* coinbase, const uchar* data) {
    char* buf;
...
static void submit_node_block(ckpool_t* ckp, sdata_t* sdata, json_t* val) {
    ...
    JSON_CPACK(
        bval, "{si,ss,ss,sI,ss,ss,si,ss,sI,sf,ss,ss,ss,ss}", "height", wb->height, "blockhash", blockhash, "confirmed",
        "n", "workinfoid", wb->id, "enonce1", enonce1, "nonce2", nonce2, "version_mask", version_mask, "nonce", nonce,
...
static void update_subscribe(ckpool_t* ckp, const char* cmd) {
    ...
    if (ckp->nonce2length && proxy->enonce2varlen != ckp->nonce2length)
        LOGWARNING(
            "Only able to set nonce2len %d of requested %d on proxy %d:%d", proxy->enonce2varlen, ckp->nonce2length, id,
            subid);
    json_decref(val);
...
static stratum_instance_t* __stratum_add_instance(ckpool_t* ckp, int64_t id, const char* address, int server) {
    sdata_t*            sdata = ckp->sdata;
    stratum_instance_t* client;
    int64_t             pass_id;
...
static void stratum_add_send(sdata_t* sdata, json_t* val, const int64_t client_id, const int msg_type) {
    ckpool_t* ckp = sdata->ckp;
    int64_t   remote_id;
    smsg_t*   msg;
...
static void stratum_broadcast_message(sdata_t* sdata, const char* msg) {
    ...
    JSON_CPACK(json_msg, "{sosss[s]}", "id", json_null(), "method", "client.show_message", "params", msg);
...
static void request_reconnect(sdata_t* sdata, const char* cmd) {
    ...
    if (port)
        url = strsep(&port, ",");
    if (url && port) {
        JSON_CPACK(json_msg, "{sosss[ssi]}", "id", json_null(), "method", "client.reconnect", "params", url, port, 0);
    } else
        JSON_CPACK(json_msg, "{sosss[]}", "id", json_null(), "method", "client.reconnect", "params");
...
static json_t* worker_stats(const worker_instance_t* worker) {
    ...
    JSON_CPACK(
        val, "{ss,ss,ss,ss,ss}", "hashrate1m", suffix1, "hashrate5m", suffix5, "hashrate1hr", suffix60, "hashrate1d",
...
static json_t* user_stats(const user_instance_t* user) {
    ...
    JSON_CPACK(
        val, "{ss,ss,ss,ss,ss,sI,sI}", "hashrate1m", suffix1, "hashrate5m", suffix5, "hashrate1hr", suffix60,
        "hashrate1d", suffix1440, "hashrate7d", suffix10080, "shares", user->shares, "authorised", user->auth_time);
    return val;
}
...
static void broadcast_ping(sdata_t* sdata) {
    ...
    JSON_CPACK(json_msg, "{s:[],s:i,s:s}", "params", "id", 42, "method", "mining.ping");
...
static json_t* userinfo(const user_instance_t* user) {
    ...
    JSON_CPACK(
        val, "{ss,si,si,sf,sf,sf,sf,sf,sf,si}", "user", user->username, "id", user->id, "workers", user->workers,
...
static json_t* workerinfo(const user_instance_t* user, const worker_instance_t* worker) {
    ...
    JSON_CPACK(
        val, "{ss,ss,si,sf,sf,sf,sf,si,sf,si,sb}", "user", user->username, "worker", worker->workername, "id", user->id,
...
static json_t* json_proxyinfo(const proxy_t* proxy) {
    ...
    JSON_CPACK(
        val, "{si,si,si,sf,ss,ss,ss,ss,ss,si,si,si,si,sb,sb,sI,sI,sI,sI,sI,si,sb,sb,si}", "id", proxy->id, "subid",
...
static void get_poolstats(sdata_t* sdata, int* sockd) {
    ...
    JSON_CPACK(
        val, "{si,si,si,si,si,sI,sf,sf,sf,sf,sI,sI,sf,sf,sf,sf,sf,sf,sf}", "start", stats->start_time.tv_sec, "update",
    ...
    do {
        ...
        if (end_t - sdata->update_time >= ckp->update_interval) {
            ...
            if (!ckp->proxy) {
                ...
                LOGDEBUG("%ds elapsed in strat_loop, pinging miners", ckp->update_interval);
                broadcast_ping(sdata);
            }
        }
...
static json_t* parse_subscribe(stratum_instance_t* client, const int64_t client_id, const json_t* params_val) {
    ...
    JSON_CPACK(ret, "[[[s,s]],s,i]", "mining.notify", sessionid, client->enonce1, n2len);
    ck_runlock(&sdata->workbase_lock);
...
static void client_auth(ckpool_t* ckp, stratum_instance_t* client, user_instance_t* user, const bool ret) {
    if (ret) {
...
static void stratum_send_diff(sdata_t* sdata, const stratum_instance_t* client) {
    ...
    JSON_CPACK(json_msg, "{s[I]soss}", "params", client->diff, "id", json_null(), "method", "mining.set_difficulty");
...
/* Needs to be entered with client holding a ref count. */
static void
add_submit(ckpool_t* ckp, stratum_instance_t* client, const double diff, const bool valid, const bool submit) {
...
static void test_blocksolve(
    ...
    send_node_block(
        ckp, sdata, client->enonce1, nonce, nonce2, ntime32, version_mask, wb->id, diff, client->id, coinbase, cblen,
        data);
...
static void submit_share(
    ...
    JSON_CPACK(
        json_msg, "{sIsssssssIsIsi}", "jobid", jobid, "nonce2", enonce2, "ntime", ntime, "nonce", nonce, "client_id",
        client->id, "proxy", client->proxyid, "subproxy", client->subproxyid);
...
/* Needs to be entered with client holding a ref count. */
static json_t* parse_submit(stratum_instance_t* client, json_t* json_msg, const json_t* params_val, json_t** err_val) {
...
static json_t* __stratum_notify(const workbase_t* wb, const bool clean) {
    ...
    JSON_CPACK(
        val, "{s:[ssssosssb],s:o,s:s}", "params", wb->idstring, wb->prevhash, wb->coinb1, wb->coinb2,
        json_deep_copy(wb->merkle_array), wb->bbversion, wb->nbit, wb->ntime, clean, "id", json_null(), "method",
        "mining.notify");
    return val;
}
...
static json_t* __user_notify(const workbase_t* wb, const user_instance_t* user, const bool clean) {
    ...
    JSON_CPACK(
        val, "{s:[ssssosssb],s:o,s:s}", "params", wb->idstring, wb->prevhash, wb->coinb1, userwb->coinb2,
        json_deep_copy(wb->merkle_array), wb->bbversion, wb->nbit, wb->ntime, clean, "id", json_null(), "method",
        "mining.notify");
    return val;
}
...
create_json_params(const int64_t client_id, const json_t* method, const json_t* params, const json_t* id_val) {
 ...
 * client holding a ref count. */
static void suggest_diff(ckpool_t* ckp, stratum_instance_t* client, const char* method, const json_t* params_val) {
...
static void send_node_all_txns(sdata_t* sdata, const stratum_instance_t* client) {
    ...
    if (client->trusted) {
        JSON_CPACK(val, "{ss,so}", "method", stratum_msgs[SM_TRANSACTIONS], "transaction", txn_array);
    } else {
        JSON_CPACK(val, "{ss,so}", "node.method", stratum_msgs[SM_TRANSACTIONS], "transaction", txn_array);
    }
    msg = ckzalloc(sizeof(smsg_t));
    msg->json_msg = val;
    msg->client_id = client->id;
...
static void parse_method(
    ...
    if (cmdmatch(method, "mining.configure")) {
        ...
        val = json_object();
        JSON_CPACK(result_val, "{sbss}", "version-rolling", json_true(), "version-rolling.mask", version_str);
        json_object_set_new_nocheck(val, "result", result_val);
        json_object_set_nocheck(val, "id", id_val);
        json_object_set_new_nocheck(val, "error", json_null());
...
static void parse_authorise_result(ckpool_t* ckp, sdata_t* sdata, stratum_instance_t* client, json_t* val) {
    if (!json_is_true(val)) {
...
static void
send_auth_response(sdata_t* sdata, const int64_t client_id, const bool ret, json_t* id_val, json_t* err_val) {
...
static void send_auth_success(ckpool_t* ckp, sdata_t* sdata, stratum_instance_t* client) {
    ...
    ASPRINTF(&buf, "Authorised, welcome to %s %s!", ckp->name, client->user_instance->username);
    stratum_send_message(sdata, client, buf);
    free(buf);
}
...
static void
parse_remote_auth(ckpool_t* ckp, sdata_t* sdata, json_t* val, stratum_instance_t* remote, const int64_t remote_id) {
...
static void send_transactions(ckpool_t* ckp, json_params_t* jp) {
    const char *        msg = json_string_value(jp->method), *params = json_string_value(json_array_get(jp->params, 0));
...
static void* statsupdate(void* arg) {
    ...
    while (42) {
        ...
        while ((user = next_user(sdata, user)) != NULL) {
            ...
            JSON_CPACK(
                val, "{ss,ss,ss,ss,ss,si,si,sI,sf,sI, sI}", "hashrate1m", suffix1, "hashrate5m", suffix5, "hashrate1hr",
            ...
            while ((worker = next_worker(sdata, user, worker)) != NULL) {
                ...
                JSON_CPACK(
                    wval, "{ss,ss,ss,ss,ss,ss,si,sI,sf,sI}", "workername", worker->workername, "hashrate1m", suffix1,
            ...
            s = json_dumps(
                val, JSON_NO_UTF8 | JSON_PRESERVE_ORDER | JSON_EOL | JSON_REAL_PRECISION(16) | JSON_INDENT(1));
        ...
        JSON_CPACK(
            val, "{si,si,si,si,si,si}", "runtime", diff.tv_sec, "lastupdate", now.tv_sec, "Users",
        ...
        JSON_CPACK(
            val, "{ss,ss,ss,ss,ss,ss,ss}", "hashrate1m", suffix1, "hashrate5m", suffix5, "hashrate15m", suffix15,
        ...
        JSON_CPACK(
            val, "{sf,sI,sI,sI,sf,sf,sf,sf}", "diff", percent, "accepted", stats->accounted_diff_shares, "rejected",
        ...
        if (ckp->proxy && sdata->proxy) {
            ...
            mutex_lock(&sdata->proxy_lock);
            JSON_CPACK(
                val, "{sI,si,si}", "current", sdata->proxy->id, "active", HASH_COUNT(sdata->proxies), "total",
                sdata->proxy_count);
            mutex_unlock(&sdata->proxy_lock);
            ...
            HASH_ITER(hh, sdata->proxies, proxy, proxytmp) {
                JSON_CPACK(
                    val, "{sI,si,sI,sb}", "id", proxy->id, "subproxies", proxy->subproxy_count, "clients",
                ...
                HASH_ITER(sh, proxy->subproxies, subproxy, subtmp) {
                    JSON_CPACK(
                        val, "{sI,si,si,sI,sI,sf,sb}", "id", subproxy->id, "subid", subproxy->subid, "nonce2len",
        ...
        sprintf(cdfield, "%lu,%lu", ts_now.tv_sec, ts_now.tv_nsec);
        JSON_CPACK(
            val, "{ss,si,si,si,sf,sf,sf,sf,ss,ss,ss,ss}", "poolinstance", ckp->name, "elapsed", diff.tv_sec, "users",
            stats->users + stats->remote_users, "workers", stats->workers + stats->remote_workers, "hashrate", ghs1,
            "hashrate5m", ghs5, "hashrate1hr", ghs60, "hashrate24hr", ghs1440, "createdate", cdfield, "createby",
            "code", "createcode", __func__, "createinet", ckp->serverurl[0]);
        json_decref(val);
