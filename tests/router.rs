...
async fn router_rejects_incompatible_resumed_enonce1() {
    ...
    add_and_activate_order(
        ...
        &wallet_bitcoind,
        &funding_descriptor,
        &format!("{}@{}", upstream_username, pool.stratum_endpoint()),
        HashDays::new(1e5).unwrap(),
        hash_price,
    )
    .await;
    ...
    let events_a = client_a.connect().await.unwrap();

    let bogus = Extranonce::random(8);
    let (subscribe_a, _, _) = client_a
        .subscribe_with_enonce1(Some(bogus.clone()))
        .await
        .unwrap();

    assert_ne!(subscribe_a.enonce1, bogus);
    ...
    let events_b = client_b.connect().await.unwrap();
    ...
    assert_eq!(subscribe_b.enonce1.len(), 4 + ENONCE1_EXTENSION_SIZE);
...
#[tokio::test]
#[timeout(120000)]
async fn router_trim_reconnects_to_intended_order() {
    let pool_bitcoind = bitcoind();
    let wallet_bitcoind = spawn_regtest();
    let descriptor = generate_descriptor();
    let funding_descriptor = generate_descriptor();
    ...
    let order_a = add_and_activate_order(
        &router,
        &wallet_bitcoind,
        &funding_descriptor,
        &format!("{}@{}", upstream_username, pool_a.stratum_endpoint()),
        HashDays::new(1e3).unwrap(),
        hash_price,
    )
    .await;
    ...
    timeout(Duration::from_secs(30), async {
        loop {
            if let Ok(detail) = router.get_order(order_a).await
                && detail.sessions.len() == 3
            {
                break;
            }
            sleep(Duration::from_millis(100)).await;
    ...
    .await
    .expect("Timeout waiting for miners to land on first order");

    let order_b = add_and_activate_order(
        &router,
        &wallet_bitcoind,
        &funding_descriptor,
        &format!("{}@{}", upstream_username, pool_b.stratum_endpoint()),
        HashDays::new(1e6).unwrap(),
        hash_price,
    )
    .await;
    ...
    timeout(Duration::from_secs(60), async {
        loop {
            if let Ok(detail) = router.get_order(order_b).await
                && detail.sessions.len() == 3
            {
                break;
            }
    ...
    .await
    .expect("Timeout waiting for trimmed miners to land on intended order");
...
async fn greet_drops_concurrent_probes_independently() {
    ...
    let mut stream_a = tokio::net::TcpStream::connect(router.stratum_endpoint())
        .await
        .unwrap();
    let mut stream_b = tokio::net::TcpStream::connect(router.stratum_endpoint())
        .await
        .unwrap();
    ...
    timeout(Duration::from_secs(5), async {
        ...
        let mut buf = [0u8; 1];
        assert_eq!(stream_a.read(&mut buf).await.unwrap(), 0);
        assert_eq!(stream_b.read(&mut buf).await.unwrap(), 0);
    })
    .await
    .expect("Timeout waiting for probes to be dropped concurrently");

    assert!(
        start.elapsed() < Duration::from_secs(5),
        "probes were dropped serially: {:?}",
        start.elapsed()
    );
